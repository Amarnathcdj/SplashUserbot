...
async def amireallyalive(alive):
    ...
    if ALV_PIC:
        tele = f"**Welcome To SplashUserBot **\n\n"
        tele += f"`{CUSTOM_ALIVE}`\n\n"
        tele += (
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
        )
        tele += f"{telemoji} **SplashBot Version**: `{telever}`\n"
        tele += f"{telemoji} **More Info**: @SplashUserbothelpchat\n"
        tele += f"{telemoji} **Sudo** : `{sudo}`\n"
        tele += f"{telemoji} **SplashBot Uptime**: `{uptime}`\n"
